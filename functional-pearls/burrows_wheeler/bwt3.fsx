...
let transform' (xs: 'a list) =
    let tag xs = List.append xs ['~']
    let n = List.length xs
    let ts = tails (tag xs) |> Seq.take n |> Seq.toList
    let pa = List.map snd (List.sort (List.zip ts [0.. (n-1)])) |> List.toArray
    let xa = rightRotate xs |> List.toArray
    
    let k = Seq.length (Seq.takeWhile (fun i -> i <> 0) pa)
    let ys = [for i in [0..(n-1)] do
              yield Array.get xa (Array.get pa i)]
    (k, ys)
