...
let comb1 ((e1:Expr), (v1:Value)) ((e2:Expr), (v2:Value)) : (Expr * Value) list =
    (if non Sub e1 && non Sub e2 then
        if non Add e2 then [(App(Add, e1, e2), v1 + v2)] else [] 
        @
        [(App(Sub, e2, e1), v2 - v1)]
     else []) 
    @
    (if 1 < v1 && non Div e1 && non Div e2 then
        ...
        @ 
        if r = 0 then [(App(Div, e2, e1), q)] else []
     else [])

let comb2 ((e1:Expr), (v1:Value)) ((e2:Expr), (v2:Value)) : (Expr * Value) list =
    (if non Sub e1 && non Add e2 && non Sub e2 then
        [(App(Add, e1, e2), v1 + v2)] 
     else [])
    @
    (if 1 < v1 && non Div e1 && non Div e2 then 
...
let countdown3 n ints = subseqs ints |> concatMap mkExprs |> nearest n
