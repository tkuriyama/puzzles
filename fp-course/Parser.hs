...
instance Functor Parser where
  ...
  (<$>) f p = P $ \s -> onResult (parse p s) k
    where k i a = Result i (f a)
...
(|||) p1 p2 = P $ \s -> let r = parse p1 s
                        in case isErrorResult r of
                             True -> parse p2 s
                             _ -> r
...
instance Monad Parser where
  ...
  (=<<) m p = P $ \s -> onResult (parse p s) k
    where k i a = parse (m a) i
...
list1 p = lift2 (:.) (P $ \s -> parse p s) (list p)
...
satisfy p = P f
  where f Nil = UnexpectedString Nil
        f (x :. xs) = if p x then Result xs x
...
surnameParser = lift2 (++) required (list lower)
  where required = lift2 (:.) upper (thisMany 5 lower)
