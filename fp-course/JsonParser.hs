...
specialChar = P f
  where f Nil = UnexpectedString Nil
        f (x :. xs) = case toSpecialCharacter x of
                        Full sc -> Result xs (fromSpecialCharacter sc)
                        Empty -> UnexpectedChar x
                           
jsonString :: Parser Chars
jsonString = between (is '"') (charTok '"') ps
  where ps = list (specialChar' ||| character')
        specialChar' = is '\\' *> (specialChar ||| hexu)
        character' = noneof "\"\\"
...
jsonObject = betweenSepbyComma '{' '}' pair
  where pair = lift2 (,) (spaces *> jsonString <* charTok ':') jsonValue
